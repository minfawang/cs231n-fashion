...
def parse_dataset(_dataset, _outdir, _max=10000):
    ...
    return _fnames_urls[:_max] if _max > 0 else _fnames_urls


if __name__ == '__main__':
    datasets = [
        ('data/train.json', 'data/train_images/', -1),
        ('data/validation.json', 'data/validation_images/', -1),
        ('data/test.json', 'data/test_images/', -1),
    ]

    for dataset, outdir, max_download in datasets:
        print('Downloading images from: {}'.format(dataset))
        print('Download to: {}'.format(outdir))
        if not os.path.exists(outdir):
            os.makedirs(outdir)

        # parse json dataset file
        fnames_urls = parse_dataset(dataset, outdir, max_download)
